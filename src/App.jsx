...
/* ---------------------------------------------------------
   API integration — เชื่อมกับ qc-backend (Node/Express + Postgres)
   แก้ API_BASE ถ้า backend รันอยู่คนละที่ (เช่น deploy ขึ้น server จริง)
--------------------------------------------------------- */
const API_BASE = "http://localhost:4000/api";
const DEMO_LINES = ["ไลน์ 1", "ไลน์ 2", "ไลน์ 3"];
...
async function api(path, { method = "GET", body, token } = {}) {
  ...
  try {
    ...
    throw { status: 0, message: `เชื่อมต่อ backend ไม่ได้ (${API_BASE}) — ตรวจสอบว่า docker compose up ไว้อยู่หรือยัง` };
  }
...
// แปลงรูปแบบข้อมูลล็อตจาก backend (flat fields) ให้เข้ากับ UI เดิม (nested incoming/final)
function normalizeLot(l) {
  return {
    ...l,
    incoming: l.incomingResult ? { result: l.incomingResult, checkedAt: l.incomingCheckedAt } : null,
    final: l.finalResult
      ? { result: l.finalResult, passCount: l.finalPassCount, failCount: l.finalFailCount }
      : null,
    spcReadings: l.spcReadings || [],
    ncr: l.ncr || null,
  };
}

// ล็อตยังไม่มีในระบบ (ฐานข้อมูลใหม่) → สร้างล็อตตัวอย่างจากสเปคสินค้าที่ seed ไว้ให้อัตโนมัติ
async function ensureDemoLots(token) {
  const specs = await api("/specs", { token });
  const today = new Date().toISOString().slice(2, 10).replace(/-/g, "");
  for (let i = 0; i < specs.length; i++) {
    const id = `LOT-${today}-${String(i + 1).padStart(2, "0")}`;
    try {
      await api("/lots", { method: "POST", token, body: { id, sku: specs[i].sku, line: DEMO_LINES[i % DEMO_LINES.length] } });
    } catch (e) {
      // ล็อตนี้อาจมีอยู่แล้ว (409) ข้ามไปได้เลย
    }
  }
}
...
function TicketHeader({ lots, shiftLabel, onLogout }) {
        ...
        <div className="flex items-center gap-2">
          ...
          {onLogout && (
            <button
              onClick={onLogout}
              className="qc2-display text-[11px] font-semibold px-2.5 py-1 rounded-sm"
              style={{ background: "var(--paper-edge)", color: "var(--ink-600)" }}
            >
              ออกจากระบบ
            </button>
          )}
  ...
  return (
    <div className="px-4 py-10 space-y-5">
      <div className="text-center space-y-1 mb-2">
        <div className="qc2-display text-lg font-bold tracking-wide" style={{ color: "var(--ink-900)" }}>เข้าสู่ระบบ QC LINE</div>
        <div className="text-[12px]" style={{ color: "var(--ink-400)" }}>เชื่อมต่อกับ backend จริงที่ {API_BASE}</div>
      </div>

      <Sheet className="space-y-3">
        <div>
          <label className="qc2-mono text-[11px] tracking-wide" style={{ color: "var(--ink-400)" }}>Username</label>
          <div className="flex items-center gap-2 rounded-md px-3 py-3 mt-1" style={{ background: "var(--paper)", border: "1px dashed var(--paper-edge)" }}>
            <input
              value={username}
        ...
        <div>
          <label className="qc2-mono text-[11px] tracking-wide" style={{ color: "var(--ink-400)" }}>Password</label>
          <div className="flex items-center gap-2 rounded-md px-3 py-3 mt-1" style={{ background: "var(--paper)", border: "1px dashed var(--paper-edge)" }}>
            <input
              type="password"
              value={password}
        ...
        {errorMsg && <div className="text-[12px] font-medium" style={{ color: "var(--stamp-red)" }}>{errorMsg}</div>}
        <Btn tone="blue" icon={ClipboardCheck} disabled={loading} onClick={() => onLogin(username, password)}>
          {loading ? "กำลังเข้าสู่ระบบ..." : "เข้าสู่ระบบ"}
        </Btn>
        <div className="text-[11px] text-center" style={{ color: "var(--ink-400)" }}>
          บัญชีตั้งต้น: admin / Admin1234! (จาก npm run seed)
        </div>
      </Sheet>
    </div>
  );
...
export default function AppV2() {
  ...
  const handleApiError = (err) => {
    if (err.status === 401) {
      setToken(null);
      showToast("เซสชันหมดอายุ กรุณาเข้าสู่ระบบใหม่", "red");
    } else {
      showToast(err.message || "เกิดข้อผิดพลาด", "red");
    }
  };

  const loadLots = async (tk) => {
    setDataLoading(true);
    try {
      let data = await api("/lots", { token: tk });
      if (data.length === 0) {
        await ensureDemoLots(tk);
        data = await api("/lots", { token: tk });
      }
  ...
  const handleLogout = async () => {
    try { await api("/logout", { method: "POST", token }); } catch (e) {}
    setToken(null);
    setLots([]);
    setScreen("home");
  };
    ...
    return (
      <div className="qc2-root w-full h-full flex justify-center" style={{ background: "#DCD4BE" }}>
        ...
        <div className="w-full max-w-[430px] h-full flex flex-col shadow-2xl" style={{ background: "var(--paper)", minHeight: 720 }}>
          <TicketHeader lots={[]} shiftLabel="กะเช้า · โรงงานผลิต A · 08:00–16:00" />
          <div className="flex-1 overflow-y-auto pb-4 qc2-grain">
            <LoginScreen onLogin={handleLogin} loading={authLoading} errorMsg={authError} />
          </div>
          <Toast toast={toast} />
        </div>
      </div>
  ...
  if (dataLoading && lots.length === 0) {
    body = <div className="px-4 py-10 text-center text-[13px]" style={{ color: "var(--ink-400)" }}>กำลังโหลดข้อมูลจาก backend...</div>;
  } else if (screen === "home") {
    ...
    body = (
      <IncomingResultScreen
        ...
        onSubmit={async (result) => {
          try {
            await api(`/lots/${activeLot.id}/incoming`, { method: "POST", token, body: { result } });
            await loadLots(token);
    ...
    body = (
      <InProcessScreen
        ...
        onReading={async (reading) => {
          try {
            const res = await api(`/lots/${activeLot.id}/spc-readings`, { method: "POST", token, body: { value: reading.value } });
            await loadLots(token);
            if (res.outOfRange) {
              setPendingReading({ value: reading.value, inRange: false });
    ...
    body = (
      <AlertScreen
        ...
        onStopLine={async () => {
          try {
            await api(`/lots/${activeLot.id}/decision`, { method: "POST", token, body: { decision: "held" } });
            await loadLots(token);
            showToast("สั่งหยุดไลน์แล้ว — สร้างเคส NCR", "red");
            setScreen("ncr");
          } catch (err) { handleApiError(err); }
        }}
      />
    ...
    body = (
      <FinalScreen
        ...
        onSubmit={async (final) => {
          try {
            await api(`/lots/${activeLot.id}/final`, { method: "POST", token, body: { passCount: final.passCount, failCount: final.failCount } });
            await loadLots(token);
            setScreen("decision");
          } catch (err) { handleApiError(err); }
        }}
    ...
    body = (
      <DecisionScreen
        ...
        onDecide={async (stage) => {
          try {
            await api(`/lots/${activeLot.id}/decision`, { method: "POST", token, body: { decision: stage } });
            await loadLots(token);
    ...
    body = (
      <NCRScreen
        lots={lots}
        showToast={showToast}
        updateNCR={async (id, patch) => {
          try {
            await api(`/ncr/${id}`, { method: "PUT", token, body: patch });
            await loadLots(token);
          } catch (err) { handleApiError(err); }
        }}
      />
  ...
  return (
    <div className="qc2-root w-full h-full flex justify-center" style={{ background: "#DCD4BE" }}>
      <style>{FONTS}</style>
      <div className="w-full max-w-[430px] h-full flex flex-col shadow-2xl" style={{ background: "var(--paper)", minHeight: 720 }}>
        <TicketHeader lots={lots} shiftLabel="กะเช้า · โรงงานผลิต A · 08:00–16:00" onLogout={handleLogout} />
        <div className="flex-1 overflow-y-auto pb-4 qc2-grain">
          {body}
        </div>
        <Toast toast={toast} />
